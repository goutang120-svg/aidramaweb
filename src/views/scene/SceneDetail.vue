<template>
  <div class="scene-detail">
    <div class="page-header">
      ...
      <h2 class="page-title">{{ scene?.sceneName || '场景详情' }}</h2>
    </div>

    <el-empty v-if="!scene && !loading" description="未找到该场景" />
        ...
        <el-tab-pane label="基础信息" name="basic">
          <el-card class="info-card">
            <el-descriptions :column="2" border>
              <el-descriptions-item label="场景名称">{{ scene?.sceneName }}</el-descriptions-item>
              <el-descriptions-item label="地点">{{ scene?.location || '-' }}</el-descriptions-item>
              ...
              <el-descriptions-item label="氛围">{{ scene?.atmosphere || '-' }}</el-descriptions-item>
              <el-descriptions-item label="状态">
                <el-tag :type="scene?.status === 'ACTIVE' ? 'success' : 'info'" size="small">
                  {{ scene?.status === 'ACTIVE' ? '启用' : '停用' }}
                </el-tag>
              </el-descriptions-item>
              <el-descriptions-item label="描述" :span="2">{{ scene?.description || '-' }}</el-descriptions-item>
            </el-descriptions>
          </el-card>
        </el-tab-pane>
...
async function fetchAssets(sceneId: number) {
  try {
    const res = await getAssets({ sceneId, page: 1, pageSize: 200 })
...
async function customUpload(options: UploadRequestOptions) {
  ...
  try {
    ...
    await fetchAssets(scene.value.id)
...
function weatherLabel(w?: string) {
  const map: Record<string, string> = { SUNNY: '晴天', CLOUDY: '多云', RAINY: '雨天', SNOWY: '雪天', WINDY: '大风' }
  return map[w || ''] || w || '-'
}
...
.info-card { background: var(--bg-white); border: 1px solid var(--border-hairline); margin-bottom: 16px; }
...
.shot-item {
  ...
  padding: 8px 12px; background: var(--bg-white); border: 1px solid var(--border-hairline);
